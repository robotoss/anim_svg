...
### Phase 4 — Visibility-aware native disposal ✅ (shipped, but defaulted **off** — see decision #9)
- [x] `pubspec.yaml` — `visibility_detector: ^0.4.0`
- [x] `AnimSvgView` API: `disposeWhenInvisible: bool` (default flipped from `true` → **`false`** in v0.0.4), `disposeDelay: 700ms`, `showDelay: 150ms`
- [x] `_AnimSvgViewState` — `VisibilityDetector(key: ObjectKey(this))` wrapper, `_renderEnabled` flag, `_hideTimer`/`_showTimer` debounce, `TickerMode.of(context)` integration in `didChangeDependencies`, `_engine` nulled on hide
- [x] Conditional render: when `_renderEnabled == false`, builds `SizedBox.shrink` instead of `tvg.Lottie.memory` — Flutter unmounts the inner Lottie subtree, which calls our existing `controller.dispose()` chain
- [x] README — "Off-screen disposal" section with tuning table, known limitations, **and FD-leak warning block linking flutter/flutter#94916**

### Phase 4.5 — SurfaceProducer migration (in progress)
Rationale: enabling Phase 4 on long, fast-scrolling lists triggers a well-known Android+Flutter `SurfaceTexture` FD-leak (flutter/flutter#94916, flutter-webrtc/flutter-webrtc#1948). Two fixes already shipped in `thorvg.flutter` were necessary but insufficient: (1) two-phase teardown moving `entry.release()` to the platform thread (`26e78c8`), (2) per-handle `ANativeWindow` cache eliminating per-frame `ANativeWindow_fromSurface`/`_release` binder churn (`4e546a0`). The residual leak is structural — `SurfaceTexture`'s `BufferQueue` holds fence FDs that drain through SurfaceFlinger's single-threaded transaction commit path; rapid create/destroy cycles outpace reclamation and exhaust the per-process FD budget after ~3 minutes.

Migration target: `TextureRegistry.createSurfaceProducer()` (Flutter 3.22 landed, **3.24 stable**, 3.27 hardened). On API 28+ this transparently selects `ImageReader`/`HardwareBuffer` instead of `SurfaceTexture` — no `BufferQueue`, no fence-FD pipeline, no leak. On API < 28 the engine falls back to `SurfaceTexture` (no regression, same risk as today). Bonus: `createSurfaceProducer` is the only path compatible with Impeller/Vulkan, which Flutter is moving toward as default.

Plan (commits, in order):
- [ ] **C.1** — bump min Flutter to `>=3.24.0` in `pubspec.yaml` + `thorvg.flutter/pubspec.yaml`. Breaking change; minor version bump (`0.0.4` → `0.1.0`).
- [ ] **C.2** — refactor `thorvg.flutter/android/.../ThorvgTexture.kt`: `entry: SurfaceTextureEntry` → `producer: SurfaceProducer`; `entry.surfaceTexture().setDefaultBufferSize` → `producer.setSize`; new `producer.setCallback({ onSurfaceAvailable, onSurfaceDestroyed })` to handle engine-driven surface destroy/recreate (e.g. backgrounding cycle). JNI bridge keeps existing `nativeAttachSurface`/`nativeDetachSurface` — only the call sites move into the new callbacks.
- [ ] **C.3** — smoke test on API 28+ device: 10-min scroll on 24-tile example, monitor `adb shell ls /proc/<pid>/fd | wc -l` for FD growth; backgrounding/foreground cycle; API <28 emulator regression check.
- [ ] **C.4** — flip `disposeWhenInvisible` default back to `true`. Update README warning to note residual API <28 risk only.
- [ ] **C.5** — close this entry and decision #9 in section 5.
...
| 7 | 2026-04-26 | Add `renderScale` parameter; default to `1.0` instead of the legacy `1 + (DPR-1) × 0.75` formula. | Even with #5 + #6 the SW backend can't sustain 60 FPS for 8 visible animations at full DPR (e.g. 2.5× emulator). Width vs height asymmetry confirmed in C++ (`tvgFlutterLottieAnimation.cpp:143-156`): for portrait Lottie, `scale = height / psize[1]` and width only adds lateral padding. `renderScale: 1.0` cuts buffer area ~6× on a 2.5× DPR device. Surface as a public knob on both `Lottie.*` and `AnimSvgView.*`. | (committed in `3e86f31`) |
| 8 | 2026-04-26 | Add visibility-aware native disposal to `AnimSvgView`: tear down handle + texture when fully off-screen for `disposeDelay`, re-create after `showDelay`. | User reported still-significant render memory residency: each mounted instance holds a scene + RGBA buffer (~720 KB at 2× DPR for 300×300) + platform texture (Android `SurfaceTextureEntry`, iOS `CVPixelBufferPool` ≈ 1 MB). `ListView.builder`'s `cacheExtent` already unmounts items past 250 logical px, but items within `cacheExtent` yet not actually visible (just-scrolled-past, partially clipped, inactive `TabBarView` tab) keep their native handle alive. Wrapping with `VisibilityDetector` and conditionally collapsing the inner `tvg.Lottie.memory` subtree to `SizedBox.shrink` triggers the existing dispose chain (`_LottieState.dispose → controller.dispose → MethodChannel "dispose" → native teardown`). Symmetric debounce (`disposeDelay: 700ms` / `showDelay: 150ms`) prevents native-create churn during fast scrolls. `TickerMode.of(context) == false` treated as zero visibility for `TabBarView` correctness. | (this file §3 Phase 4) |
| 9 | 2026-04-26 | Flip `disposeWhenInvisible` default `true` → `false` in v0.0.4. Defer real fix to SurfaceProducer migration (Phase 4.5). | Phase 4 amplifies a structural Android `SurfaceTexture` FD-leak (`createSurfaceTexture` → `BufferQueue` → fence FDs drained asynchronously through SurfaceFlinger). Symptoms exactly match flutter/flutter#94916 + flutter-webrtc/flutter-webrtc#1948: rapid create/destroy on a fast-scrolled list exhausts per-process FD budget within ~3 min, raster thread aborts inside `SurfaceTexture.updateTexImage` with `error dup'ing fence fd`. Two earlier patches (two-phase teardown `26e78c8`, per-handle `ANativeWindow` cache `4e546a0`) reduced the leak rate but couldn't eliminate it inside the `SurfaceTexture` API. Two paths considered: (a) soft-pause keeping `SurfaceTextureEntry` alive but releasing the thorvg scene + RGBA buffer (~80 LoC, partial mem release, throwaway code once SurfaceProducer lands), (b) full migration to `createSurfaceProducer` (Flutter 3.24+, uses `ImageReader`/`HardwareBuffer` on API 28+, official Impeller-ready path). Chose (b) — but until C.2 ships the only safe default is `false`. Public API kept identical so users who explicitly opt in on short/slow lists continue to work. | flutter/flutter#94916, flutter-webrtc/flutter-webrtc#1948, this file §3 Phase 4.5 |
